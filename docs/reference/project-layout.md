...
└── ts/                        # the self-contained npm package root (@voxgig/sdkgen)
    ...
    ├── model/
    │   └── sdkgen.aon         # authoritative base model schema, shipped directly
    ...
    └── project/
        └── .sdk/              # the scaffold copied into consumer projects
            ├── model/
            │   ├── target/<lang>.aon       # target definitions
            │   └── feature/<name>.aon      # feature definitions + index
...
| Path | Layer | Becomes |
| --- | --- | --- |
| `model/target/<lang>.aon` | model | the target definition (deps, ext, module) |
| `model/feature/<name>.aon` | model | the feature definition (hooks, deps) |
...
└── .sdk/                   # shaped byte-for-byte like ts/project/.sdk
    ├── model/
    │   ├── target/<t>.aon     # one per provided target
    │   └── feature/<f>.aon    # one per provided feature
...
claim needs `model/target/<t>.aon`, `src/cmp/<t>/` **and** `tm/<t>/`; a
...
│   ├── model/
│   │   ├── api.aon         # apidef output (entities, operations, info)
│   │   ├── target/            # target defs + target-index.aon
│   │   └── feature/           # feature defs + feature-index.aon
...
or deploy. These scripts require the project's `.sdk` dependencies.
