...
| Dynamic (no typed model) | clojure, haskell, ocaml, perl, zig, lean, plus the consumer targets go-cli, go-mcp, py-data and the packaged seneca-provider | Single dynamic value type throughout; README type columns are documentary. A consumer target has no typed model of its own — it reaches types, where they exist, through the SDK it wraps. |
