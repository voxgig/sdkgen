...
| Feature hook not firing | hook not `active` in the model, or not implemented | feature `.aon` + the target's feature source (`tm/<lang>/**/feature/`) |
| Feature source missing from a generated SDK | feature not declared, or `active: false`, in the model | `model/feature/feature-index.aon` + the feature `.aon` |
