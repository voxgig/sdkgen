...
describe('generate', () => {
  ...
  test('documentation fallbacks do not warn about optional overrides', async () => {
    const sink: any[] = []
    const out = await generate(['c'], undefined, undefined, sink)
    ok(out['c/AGENTS.md'].includes('Generated by'))
    ok(out['c/README.md'].includes('## Features'))
    const missing = sink.filter((entry) => 'require-missing' === entry.point)
    ok(missing.some((entry) => entry.path.includes('NoSuchThing_c')))
    deepStrictEqual(missing.filter((entry) => /ReadmeFeatures_|AgentGuide_/.test(entry.path)), [])
  })
