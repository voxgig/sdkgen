...
describe('pointPath', () => {
  ...
  // A STALE MODEL — generated by apidef <= 8.1, never regenerated — carries
  // `parts` and no `segments`. Treating it as pathless would emit an SDK
  // whose every request goes to the API root, silently, because an empty
  // path is indistinguishable from a GraphQL point here.
  //
  // apidef 8.2.0 tried to catch this in the schema (`parts?: null`) and that
  // bricked the upgrade instead: the stale model is the INPUT to the tool
  // that rewrites it, and voxgig-model unifies before running apidef's
  // build, so the file blocked its own regeneration. The check belongs here.
  test('pointSegments: a stale `parts` model is refused, not read as pathless', () => {
    ...
    // A GraphQL point genuinely has no path and must still be fine.
