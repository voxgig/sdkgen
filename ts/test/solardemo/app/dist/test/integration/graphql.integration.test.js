...
describe('GraphQL API Integration', () => {
    ...
    test('load: planet by id', async () => {
        const { status, body } = await gql('query PlanetLoad($id: String!) { planet(id: $id) { id name kind diameter } }', { id: 'earth' });
    ...
    test('list: planets as a relay connection', async () => {
        const { body } = await gql('query { planets { nodes { id } pageInfo { hasNextPage endCursor } } }');
    ...
    test('list: paging walks to a terminating final page', async () => {
        ...
        let pages = 0;
        for (; pages < 20; pages++) {
            const { body } = await gql('query P($first: Int, $after: String) { planets(first: $first, after: $after)' +
                ' { nodes { id } pageInfo { hasNextPage endCursor } } }', { first: 2, after });
            const conn = body.data.planets;
            seen.push(...conn.nodes.map((n) => n.id));
        ...
        deepStrictEqual(seen, all.data.planets.nodes.map((n) => n.id));
    });
    test('relations: planet moons, and moon back to planet', async () => {
        const { body } = await gql('query { planet(id: "earth") { moons { nodes { id name planet { id } } } } }');
    ...
    test('create/update/delete: payload wrappers carry the entity', async () => {
        const created = await gql('mutation C($input: PlanetCreateInput!) { planetCreate(input: $input)' +
            ' { success planet { id name kind diameter } } }', { input: { name: 'Vulcan', kind: 'rock', diameter: 9000 } });
        strictEqual(created.body.data.planetCreate.success, true);
        const id = created.body.data.planetCreate.planet.id;
        ok(null != id);
        const updated = await gql('mutation U($id: String!, $input: PlanetUpdateInput!)' +
            ' { planetUpdate(id: $id, input: $input) { success planet { name } } }', { id, input: { name: 'Vulcan II' } });
        strictEqual(updated.body.data.planetUpdate.planet.name, 'Vulcan II');
        const removed = await gql('mutation D($id: String!) { planetDelete(id: $id) { success } }', { id });
        strictEqual(removed.body.data.planetDelete.success, true);
        const gone = await gql('query Q($id: String!) { planet(id: $id) { id } }', { id });
    ...
    test('actions: terraform and forbid', async () => {
        const t = await gql('mutation T($id: String!) { planetTerraform(id: $id, start: true)' +
        ...
        const f = await gql('mutation F($id: String!, $why: String)' +
            ' { planetForbid(id: $id, forbid: true, why: $why) { success state } }', { id: 'mars', why: 'hostile' });
    ...
    test('errors: execution failure rides HTTP 200', async () => {
        const { status, body } = await gql('query { planet(id: "earth") { id nope } }');
    ...
    test('parity: moon create rejects an unknown planet', async () => {
        const { body } = await gql('mutation C($input: MoonCreateInput!) { moonCreate(input: $input)' +
            ' { success } }', { input: { planetId: 'nope', name: 'M', kind: 'rock', diameter: 1 } });
        ok(0 < (body.errors ?? []).length);
        strictEqual(body.errors[0].extensions.code, 'NOT_FOUND');
    });
    ...
    test('parity: terraform without flags preserves state', async () => {
        await gql('mutation { planetTerraform(id: "venus", start: true)' +
            ' { state } }');
        const { body } = await gql('mutation { planetTerraform(id: "venus") { state } }');
    ...
    test('parity: explicit null in an update is ignored, not written', async () => {
        ...
        const { body } = await gql('mutation U($input: PlanetUpdateInput!)' +
            ' { planetUpdate(id: "earth", input: $input) { planet { name kind } } }', { input: { name: null, kind: 'gas' } });
        strictEqual(body.data.planetUpdate.planet.name, before.body.data.planet.name);
    ...
    test('parity: a stale cursor ends the walk', async () => {
        const created = await gql('mutation C($input: PlanetCreateInput!) { planetCreate(input: $input)' +
            ' { planet { id } } }', { input: { name: 'Ghost', kind: 'rock', diameter: 1 } });
        const id = created.body.data.planetCreate.planet.id;
        await gql('mutation D($id: String!) { planetDelete(id: $id) { success } }', { id });
        const { body } = await gql('query P($after: String) { planets(after: $after)' +
