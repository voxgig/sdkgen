    ...
    (0, jostraca_1.Folder)({ name: '.github' }, () => {
        (0, jostraca_1.Folder)({ name: 'workflows' }, () => {
            for (const target of npmTargets) {
                (0, jostraca_1.File)({ name: workflowFile(target) }, () => {
                    (0, jostraca_1.Content)(publishWorkflow(model, target, npmTargets, repository));
                });
            }
        });
    });
...
function workflowFile(target) {
    return `publish-${target.name}.yml`;
}
...
function publishingDoc(model, targets, repository) {
    const pkg = (0, packageMeta_1.packageName)(model, targets[0].name);
    const first = workflowFile(targets[0]);
    const setupHow = null == repository ? 'run:' :
        'run\n`.sdk/admin/setup-npm-trust.sh`. It registers every workflow in the table\n' +
            ...
            `\`${targets[0].name}\` target it runs:`;
    const setupCheck = null == repository ? '' : `
...
`;
    const rows = targets.map((t) => `| \`${t.name}/\` | ${(0, packageMeta_1.packageName)(model, t.name)} | ` +
        `\`.github/workflows/${workflowFile(t)}\` |`).join('\n');
...
3. Run the workflow from the Actions tab, or:
       ...
       gh workflow run ${first} --ref main -f expect_sha=$(git rev-parse HEAD)
...
required), ${setupHow}
    ...
    npm trust github ${pkg} \\
      ...
      --allow-publish
${setupCheck}
\`npm trust list ${pkg}\` shows the configuration, and
\`npm trust revoke ${pkg} --id=<id>\` removes one.

**The workflow filename is part of the configuration.** Renaming
\`${first}\` breaks publishing until the npm side is updated to match.
