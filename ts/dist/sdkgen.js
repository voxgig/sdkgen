...
const Docs_1 = require("./cmp/Docs");
const ExternalDocs_1 = require("./cmp/ExternalDocs");
...
function SdkGen(opts) {
    ...
    async function generate(spec) {
        ...
        const intreedocs = Object.keys(model?.main?.[apidef_1.KIT]?.docs ?? {})
            .filter((name) => {
            const item = model.main[apidef_1.KIT].docs[name];
        ...
        if (0 < intreedocs.length) {
            ...
            const dres = await jostraca.generate(jopts, () => (0, Docs_1.Docs)({ model }));
        ...
        for (const ext of external) {
            ...
            const eres = 'docs' === ext.kind ?
                await jostraca.generate({ ...jopts, folder: ext.folder }, () => (0, ExternalDocs_1.ExternalDocs)({
                    model, item: ext.target, cmpfolder: folder, sdkrelpath,
                })) :
                await jostraca.generate({ ...jopts, folder: ext.folder }, () => (0, ExternalTarget_1.ExternalTarget)({
...
function externalItems(model, folder, kinds) {
    return kinds.flatMap((kind) => {
        const items = model?.main?.[apidef_1.KIT]?.[kind] || {};
        return Object.keys(items).sort()
            .map((name) => ({ kind, name, target: items[name] }))
            .filter((t) => {
