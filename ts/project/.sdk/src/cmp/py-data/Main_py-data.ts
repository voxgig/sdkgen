...
const Main = cmp(function Main(props: any) {
  ...
  // Copy the static runtime (frames.py, fetch.py, auth.py, Makefile, LICENSE,
  // the template-side tests) with the standard ProjectName substitutions.
  Copy({
    from: 'tm/' + target.name,
    exclude: [/src\//],
    replace: { ...ctx$.stdrep },
...
# Flat single-file modules: setuptools' package finder only discovers package
# DIRECTORIES, so these must be listed explicitly or the documented import
# installs to nothing.
[tool.setuptools]
py-modules = ["${dataModule}", "entity_frames", "frames", "fetch", "auth"]
`)
  })
})
...
`
  ...
  File({ name: dataModule + '.py' }, () => Content(`# ${name} Data — notebook-friendly access to the ${name} API.
#
...
const EntityFramesTest = cmp(function EntityFramesTest(props: any) {
  const { frameEnts, seriesEnts, sdkClass, sdkModule } = props
...
class TestFramesAgainstTestMode:
    ...
    def _client(self):
        from ${sdkModule.replace(/_sdk$/, '')}_data import ${sdkClass.replace(/SDK$/, '')}Data
