...
The ${target.title} SDK for the ${model.Name} API — an entity-oriented client following idiomatic Scala conventions.
...
`)
  ...
  const example = exampleEntity
    ? `for example \`client.${scalaVarName(exampleEntity.name)}(null)\` — each`
    : 'each'
