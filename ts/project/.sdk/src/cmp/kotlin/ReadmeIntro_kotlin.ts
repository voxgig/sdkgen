...
The ${target.title} SDK for the ${model.Name} API — an entity-oriented client following idiomatic Kotlin conventions.
...
`)
  ...
  const example = exampleEntity
    ? `for example \`client.${kotlinVarName(exampleEntity.name)}(null)\` — each`
    : 'each'
