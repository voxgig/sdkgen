...
`)
    ...
    publishedEntities.map((ent: any) => {
      Content(`#### \`${kotlinVarName(ent.name)}(entopts)\`
...
`)
    ...
    publishedEntities.map((ent: any) => {
      ...
      // Sanitise the local variable name — kotlinVarName guards Kotlin keywords.
      const eVar = kotlinVarName(ent.name)
      const accessor = kotlinVarName(ent.name)
...
`)
          ...
          if ('load' === opname || 'remove' === opname) {
            ...
            const arg = 0 < matchItems.length
              ? `mutableMapOf<String, Any?>(${matchItems.map((it: any) =>
...
`)
            ...
            Content(`\`\`\`kotlin
val result = client.${accessor}(null).create(mutableMapOf<String, Any?>(
`)
...
`)
            ...
            const updateLines = updateItems.map((it: any, i: number) => {
              ...
              return `    "${it.name}" to ${kotlinLit(it.type,
...
`)
      activeFeatures.map((f: any) => {
        Content(`    "${f.name}" to mapOf("active" to true),
`)
      })
      Content(`)
val client = ${SDK}(mutableMapOf<String, Any?>("feature" to feature))
\`\`\`
