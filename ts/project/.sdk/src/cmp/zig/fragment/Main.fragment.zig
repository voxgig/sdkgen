...
pub const ProjectNameSDK = struct {
    mode: []const u8 = "live",
    options: Value = .{ .null = {} },
    // NOT named `utility`. Entity accessors are generated as methods on this
    // same struct (MainEntity_zig), so an API with an entity called `utility`
    // would collide: Zig rejects a local binding that shadows the declaration,
    // and `sdk.utility` resolves to the FIELD, which makes the generated
    // accessor unreachable. The public reader is `get_utility()`, so this name
    // is internal and free to be one an entity name cannot produce.
    util_rt: *Utility,
    ...
    pub fn new(options: Value) *ProjectNameSDK {
        ...
        const rootctx = sdk.util_rt.make_context(CtxSpec{
            .client = sdk,
            .utility = sdk.util_rt,
            .config = cfg,
            .options = options,
        ...
        const opts = sdk.util_rt.make_options(rootctx);
        sdk.options = opts;
            ...
            for (feature_order.array.data.items) |fname_v| {
                ...
                if (fopts == .object) {
                    if (h.get_bool(fopts, "active") orelse false) {
                        sdk.util_rt.feature_add(rootctx, config.make_feature(fname));
                    }
                }
            }
        }
        ...
        for (snap.items) |f| sdk.util_rt.feature_init(rootctx, f);

        sdk.util_rt.feature_hook(rootctx, "PostConstruct");
    ...
    pub fn get_utility(self: *ProjectNameSDK) *Utility {
        return Utility.copy(self.util_rt);
    }
    ...
    pub fn prepare(self: *ProjectNameSDK, fetchargs_in: Value) E!Value {
        ...
        const ctx = self.util_rt.make_context(CtxSpec{
            .opname = "prepare",
            .ctrl = ctrl,
        }, self.get_root_ctx());
        ...
        const headers = self.util_rt.prepare_headers(ctx);
        ...
        _ = try self.util_rt.prepare_auth(ctx);

        return self.util_rt.make_fetch_def(ctx);
    }
    ...
    fn raw_request(self: *ProjectNameSDK, fetchargs_in: Value) Value {
        ...
        const fetched = self.util_rt.fetch(ctx, url, fetchdef) catch {
            return h.jo(&.{
                .{ "ok", h.vbool(false) },
