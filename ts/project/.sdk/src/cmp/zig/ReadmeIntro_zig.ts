...
The ${target.title} SDK for the ${model.Name} API — an entity-oriented client following idiomatic Zig conventions.
...
`)
  ...
  const example = exampleEntity
    ? `for example \`client.${zigVarName(exampleEntity.name)}(h.vnull())\` — each`
    : 'each'
