...
`)
    ...
    if (nestedEntity) {
      ...
      const neVar = javaVarName(nestedEntity.name)
      const neAccessor = javaVarName(nestedEntity.name)
...
`)
    ...
    const examplePairs = (opname: string): string[] => {
      ...
      return chosen.map((it: any) => `"${it.name}", ${javaLit(it.type, 'example_' + it.name)}`)
    }
