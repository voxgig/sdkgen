...
const { hasFeature, makeClient, makeClock, makeResponse } = require('./feature/harness')
...
describe('feature', () => {
  ...
  if (hasFeature('telemetry')) describe('telemetry', () => {
    ...
    test('opens spans and propagates trace headers', async () => {
      const rec = recordingServer()
      const spans = []
      const h = makeClient({ features: [{ name: 'telemetry', options: { exporter: (s) => spans.push(s) } }], server: rec.server })
      const res = await h.op({ op: 'load' })
      strictEqual(res.ok, true)
  ...
  if (hasFeature('debug')) describe('debug', () => {
    ...
    test('captures a redacted trace and honours onEntry + max', async () => {
      const seen = []
      const h = makeClient({ features: [{ name: 'debug', options: { max: 1, onEntry: (e) => seen.push(e) } }] })
  ...
  if (hasFeature('proxy')) describe('proxy', () => {
    ...
    test('routes through the proxy and invokes an agent factory', async () => {
      const rec = recordingServer()
      let agentUrl = ''
      const h = makeClient({ features: [{ name: 'proxy', options: { url: 'http://proxy:8080', agent: (u) => { agentUrl = u; return { a: 1 } } } }], server: rec.server })
      await h.op({ op: 'load' })
  ...
  if (hasFeature('telemetry')) test('telemetry: injected idgen + clock', async () => {
    const h = makeClient({ features: [{ name: 'telemetry', options: { idgen: (k) => k + '-X', now: () => 5 } }] })
    await h.op({ op: 'load' })
  ...
  if (hasFeature('clienttrack')) test('clienttrack: injected idgen + fixed session', async () => {
    const rec = recordingServer()
    const h = makeClient({ features: [{ name: 'clienttrack', options: { sessionId: 'S1', idgen: (k) => k + '-1' } }], server: rec.server })
    await h.ready()
    await h.op({ op: 'load' })
