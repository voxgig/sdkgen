...
describe('pipeline:makePoint + makeSpec', () => {
  ...
  test('makePoint rejects a disallowed operation', () => {
    ...
    strictEqual(stdutil.makePoint(ctx).code, 'point_op_allow')
  })

  test('makePoint rejects an operation with no endpoints', () => {
    const ctx = base({ op: { name: 'load', points: [] }, options: { allow } })
    strictEqual(stdutil.makePoint(ctx).code, 'point_no_points')
  })
  ...
  test('makePoint returns the single point', () => {
    ...
    strictEqual(stdutil.makePoint(ctx), point)
  })

  test('makePoint short-circuits a feature-supplied point', () => {
    const preset = { method: 'GET' }
    strictEqual(stdutil.makePoint(base({ out: { point: preset } })), preset)
  })
...
describe('pipeline:makeResponse', () => {
  ...
  test('short-circuits when a feature already supplied the response', async () => {
    ...
    strictEqual(await stdutil.makeResponse(ctx), preset)
  })
})
...
describe('pipeline:makeResult', () => {
  ...
  test('short-circuits on a preset result', () => {
    ...
    strictEqual(stdutil.makeResult(base({ out: { result: preset }, spec: {}, result: {} })), preset)
  })
})
...
describe('pipeline:makeRequest', () => {
  ...
  test('guards a missing spec', async () => {
    strictEqual((await stdutil.makeRequest(base({ spec: null }))).code, 'request_no_spec')
  })
  ...
  test('a null transport result becomes a response error', async () => {
    ...
    const r = await stdutil.makeRequest(ctx)
    ok(null != r.err)
  })
  ...
  test('short-circuits a feature-supplied request', async () => {
    ...
    strictEqual(await stdutil.makeRequest(base({ out: { request: preset }, spec: {} })), preset)
  })
})
...
describe('pipeline:makeFetchDef', () => {
  ...
  test('guards a missing spec', () => {
    strictEqual(stdutil.makeFetchDef(base({ spec: null })).code, 'fetchdef_no_spec')
  })
  ...
  test('serialises an object body to JSON and inits a missing result', () => {
    ...
    const fd = stdutil.makeFetchDef(ctx)
...
describe('pipeline:makeError + done', () => {
  ...
  test('done returns resdata on success', () => {
    strictEqual(stdutil.done(base({ result: { ok: true, resdata: 42 } })), 42)
  })

  test('done throws the error when not ok', () => {
    let threw = false
    try { stdutil.done(base({ result: { ok: false } })) }
    catch (e) { threw = true }
  ...
  test('done cleans ctrl.explain on success', () => {
    ...
    strictEqual(stdutil.done(ctx), 7)
  })
  ...
  test('makeError returns resdata instead of throwing when ctrl.throw is false', () => {
    ...
    strictEqual(stdutil.makeError(ctx), 'fallback')
  })
...
describe('pipeline:featureAdd ordering', () => {
  ...
  test('appends by default', () => {
    const c = client()
    stdutil.featureAdd({ client: c, utility: stdutil }, { name: 'z', _options: {} })
    strictEqual(c._features.map((f) => f.name).join(','), 'a,b,z')
  })

  test('__before__ inserts ahead of the named feature', () => {
    const c = client()
    stdutil.featureAdd({ client: c, utility: stdutil }, { name: 'z', _options: { __before__: 'b' } })
    strictEqual(c._features.map((f) => f.name).join(','), 'a,z,b')
  })

  test('__after__ inserts behind the named feature', () => {
    const c = client()
    stdutil.featureAdd({ client: c, utility: stdutil }, { name: 'z', _options: { __after__: 'a' } })
    strictEqual(c._features.map((f) => f.name).join(','), 'a,z,b')
  })

  test('__replace__ swaps the named feature', () => {
    const c = client()
    stdutil.featureAdd({ client: c, utility: stdutil }, { name: 'z', _options: { __replace__: 'a' } })
    strictEqual(c._features.map((f) => f.name).join(','), 'z,b')
  })
})
...
describe('pipeline:feature order', () => {
  ...
  function resolve(feature) {
    const ctx = { utility: stdutil, options: { feature }, config: { options: {} } }
