...
// checks every ```lua block in the module README with load(), and (2)
  ...
  it("every runnable lua block runs offline in test mode", function()
    ...
    for i, block in ipairs(blocks) do
      if is_runnable(block) then
        ...
        assert.is_not_nil(chunk,
          "runnable block #" .. i .. " failed to compile: " .. tostring(err) ..
          "\\n----\\n" .. src .. "\\n----")
        local ok, e = pcall(chunk)
        if not ok then
          local msg = tostring(e)
          -- A nil accessor / missing method / bad index is a real README
          ...
          if msg:find("nil value") or msg:find("attempt to call")
            ...
            assert.is_true(false,
              "runnable block #" .. i .. " hit a runtime error (missing " ..
              "method / nil call): " .. msg .. "\\n----\\n" .. src .. "\\n----")
          end
    ...
    assert.is_true(ran > 0,
      "expected at least one runnable lua block in the README")
  end)
end)
`)
  })
