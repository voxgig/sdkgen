...
| Change the base model schema | `ts/model/sdkgen.aontu` (authoritative) | `make check-model build test` |
...
ts/                    the self-contained npm package root (@voxgig/sdkgen)
  ...
  model/sdkgen.aontu     authoritative base model schema, shipped directly
...
`.sdk/test/feature/<name>.aontu`, run by a generated SDK's own
